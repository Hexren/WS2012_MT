...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_ErdMond_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_ErdMond_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_ErdMond_autoinheritance_info( int nlhs, mxArray * plhs[],
  ...
  if (strcmp(machineName, "ErdMond") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_ErdMond_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_ErdMond_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "ErdMond") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_ErdMond_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_ErdMond_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"ErdMond")) {
    return(sf_ErdMond_method_dispatcher(simstructPtr,chartFileNumber,specsCksum,
            method,data));
  }
...
extern void ErdMond_terminator(void);
...
extern void ErdMond_initializer(void);
extern void ErdMond_register_exported_symbols(SimStruct* S);
extern void ErdMond_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  ErdMond_register_exported_symbols(S);
}
